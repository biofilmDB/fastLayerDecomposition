...
import Convexhull_simplification as chs
...
# this is the option to figure out number verts based on an error threshold
...
#     hull=ConvexHull(data.reshape((-1,3)), qhull_options="Qs")
    ...
    print("In Hull simplification determined version")
    ...
    print ("Number unique colors is", len(unique_data))
...
def recover_ASAP_weights_using_scipy_delaunay(Hull_vertices, data, option=1):
    ...
    assert tetrahedra[tetrahedra==-1].size == 0
...
def Get_ASAP_weights_using_Tan_2016_triangulation_and_then_barycentric_coordinates(img_label_origin, origin_order_tetra_prime, outprefix, order=0):
    """
    img_label origin is RGB points restricted to points on the 5D convex hull.
    origin_order_tetra_prime is the palette colors
    """
    ...
    # now all points are inside convex hull
    ...
    if DEMO==False:
        mixing_weights_filename=outprefix+"-RGB_ASAP-using_Tan2016_triangulation_and_then_barycentric_coordinates-linear_mixing-weights.js"
